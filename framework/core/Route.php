...
{
    // 路由规则
    //Route::rule('back/:id', 'login/index'); //一般路由
    //Route::rule('__domain__', ['zuowen1' => 'zuowen']); //二级域名路由 __domain__
    ...
    protected static function checkRule($uri, $param)
    {
        ...
        foreach (self::$rules as $rule => $route) {
            if ('__domain__' != $rule && $find_route == $route) {
    ...
    private function doRules($routerStr)
    {
        ...
        if ($routerStr === '') {
            ...
            $route_tmp = explode('/', rtrim($routerStr));
        }
        $route_arr = array_filter($route_tmp);

        // $res = self::parseVar("back/[:abc]/:id/[:is]");
        $param = [];
        $find_routeStr = '';
        $rules = self::$rules;
        unset($rules['__domain__']); // 排除二级域名路由规则
        foreach ($rules as $rule => $routeStr) {
            $items = explode('/', $rule);
    ...
    {
        //自定义路由规则检测并替换
        $route_tmp = $this->doRules($routerStr);
        $route_info = array();
